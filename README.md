...
This repository describes a **Tree Autoregressive Hidden Markov Model (Tree AR-HMM)** for modeling cell lineages, where each biological cell follows a Markovian latent state sequence with AR(1) Gaussian emissions, and division events create a branching tree structure.

The key idea:  
- Each cell has a **latent discrete state** that evolves over time.  
...
- **Within-cell transitions**
  \[
  z_{r,t+1} \mid z_{r,t} = k \sim \mathrm{Cat}(\pi_k),
...
\[
p\bigl(x_{i,t} \mid z_{i,t} = k,\; x_{i,t-1}\bigr)
  = \ell_t^{(i)}(k),
\]
for example a Gaussian AR(1):
\[
x_{i,t} \mid z_{i,t} = k,\; x_{i,t-1}
  \sim \mathcal{N}(A_k x_{i,t-1} + b_k,\; \Sigma_k).
\]
...
\[
z_{i,\tilde t + 1} \mid z_{r,\tilde t} = k \sim \mathrm{Cat}(\tilde \pi_k),
\quad i \in \{n,m\}.
\]
...
\[
z_{i,t+1} \mid z_{i,t} = k \sim \mathrm{Cat}(\pi_k),
\quad t \ge \tilde t+1,
\]
...
= \prod_{r=1}^R
  \left[
    p(z_{r,1}) 
    \prod_{t} \prod_{n \in C_{r,t}}
      p\bigl(x_{n,t} \mid z_{n,t}, x_{n,t-1}\bigr)
      \prod_{c_n} p\bigl(z_{c_n, t+1} \mid z_{n,t}\bigr)
  \right],
\]
...
Optionally, division events themselves can be modeled by appending a division indicator to the observations. (TODO still).
